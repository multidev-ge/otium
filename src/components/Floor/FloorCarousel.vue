...
const isFeatured = (floor) => floorNumber === getFloor(floor);
const isValid = (floor) => floor >= 1 && floor <= blockLength;
const getFloor = (index) => floorNumber + 3 - index;
</script>
  ...
  <div class="flex lg:justify-between items-center max-lg:flex-row-reverse max-lg:gap-x-6">
    <div class="flex lg:flex-col h-full lg:justify-between lg:items-center">
      <div
          ...
          :key="floor"
          :class="[
            isFeatured(floor) ? 'flex lg:flex-col items-center': 'max-lg:invisible',
            {'invisible': !isValid(floor)}
          ]">
        <floor-caret-up-icon
            v-if="isFeatured(floor)"
            @click="emit('nextFloor')"
            class="max-lg:rotate-180 cursor-pointer"
            :disabled="floorNumber === blockLength"
        />
        <span
            v-text="getFloor(floor)"
            @click="emit('changeFloor', getFloor(floor))"
