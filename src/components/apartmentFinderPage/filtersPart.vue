...
export default {
  ...
  computed: {
    project_id: {
      ...
      set(val) {
        this.$store.commit("flats/SET_STATE", { key: "project_id", value: val })
    ...
    block_id: {
      ...
      set(val) {
        this.$store.commit("flats/SET_STATE", { key: "block_id", value: val })
    ...
    floor_id: {
      get() {
        return this.$store.state.flats.floor_id
      },
      set(val) {
        this.$store.commit("flats/SET_STATE", { key: "floor_id", value: val })
        this.$store.dispatch('flats/getFlats')
      }
    },
    sold: {
    ...
    ...mapGetters('flats', ['blocks', ,'floors', 'requestFilters', 'rooms', 'projects']),
    // ...mapGetters('projects', ['projects']),
  },
  methods: {
    ...mapActions("flats", ['getFlats', 'getBlocks', 'clearFilter', 'getProjects']),
  ...
  <div class="md:w-1/6 w-11/12 md:mx-0 mx-auto flex flex-col font-medium  gap-10">
    ...
    <div class="flex flex-col gap-3">
      ...
      <select v-model="project_id" :disabled="!projects?.data?.length"
        ...
        <option v-for="component in projects.data" :value="component?.id" :key="component?.id">{{ component.title }}</option>
      </select>
    </div>
    <div class="flex flex-col gap-3">
    ...
    <div class="flex flex-col gap-3">
      ...
      <div class="flex gap-2">
        <div class="w-10 h-10 text-lg flex items-center justify-center border-black border rounded-md cursor-pointer"
          @click.prevent="updateRooms(index + 1)" v-for="(room, index) in Srooms.quantity" :class="[
    ...
    <TwoDotRange name="price" :displayName="t('filters.price')" :max-value="doubleSliderOptions.price.maxValue"
      ...
      :prefix="doubleSliderOptions.price.prefix" :decimal=0 thousand="," @onChange="twoChange" />
    <div class="flex flex-col gap-3">
      ...
      <div class="flex gap-10">
        ...
        <div class="flex gap-2">
          <input v-model="sold" class="w-5 h-5 accent-black rounded-full" value="0" name="hideShow" type="radio">
