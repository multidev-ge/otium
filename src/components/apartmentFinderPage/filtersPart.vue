...
<template>
  <div class="md:w-1/6 w-11/12 md:mx-0 mx-auto flex flex-col font-medium  gap-10">
    ...
    <div class="flex flex-col gap-3">
      <label class="text-xl font-normal" for="project">{{ dropDowns.project.name }}</label>
      <select 
        v-model="project_id"
        ...
        <option :value="null" disabled>{{ dropDowns.project.value }}</option>
        <option v-for="component in projects" :value="component?.id" :key="component?.id">{{ component.title }}</option>
      </select>
    </div>
    <div class="flex flex-col gap-3">
      <label class="text-xl font-normal" for="project">{{ dropDowns.block.name }}</label>
      <select 
        v-model="block"
        class="w-full py-3 rounded-md border-r-8 border-transparent px-2 text-lg outline outline-black outline-1 opacity-40"
        :class="Array.from(projects, (project) => project.id).includes(project_id) ? 'opacity-100' : 'opacity-40'">
        <option :value="null" disabled>{{ dropDowns.block.value }}</option>
    ...
    <TwoDotRange name="area" :displayName="doubleSliderOptions.area.name" :max-value="doubleSliderOptions.area.maxValue"
      :default-values="[doubleSliderOptions.area.minValue + 10, doubleSliderOptions.area.maxValue - 50]" :decimal=0
      :merge="35"
      @onChange="twoChange" 
      />
    
    <TwoDotRange name="price" :displayName="doubleSliderOptions.price.name"
