...
import blockPlan from '@/assets/images/01_00002.jpg'
...
const {
    planWidth: planWidthBlocks,
    planHeight: planHeightBlocks
} = await imageDimensions(blockPlan).then(dimensions => dimensions).catch(err => console.log(err));

const leftBlock = ref([
    {
        points: [486,2364,482,2511,1257,2511,1258,2457,1262,2456,1299,2455,1299,2447,1333,2402,1333,2406,1387,2404,1420,2351,1419,2278,1447,2243,1493,2241,1514,2208,1515,2145,1532,2129,1535,2051,1515,2074,1514,2121,1492,2146,1470,2175,1420,2175,1418,2185,1417,2245,1326,2363,486,2364],
        floor: 1,
        pos: 'left',
        details: [
...
const rightBlock = ref([
    {
        points: [3472,2404,3473,2558,2667,2558,2656,2546,2625,2497,2618,2481,2621,2476,2592,2431,2536,2432,2507,2383,2507,2297,2448,2204,2405,2202,2387,2173,2388,2109,2380,2100,2379,2020,2386,2020,2386,2084,2386,2091,2386,2094,2406,2121,2466,2212,2504,2212,2504,2212,2505,2277,2589,2404,3471,2404],
        floor: 1,
        pos: 'right',
        details: [
            { name: "Apartments", value: "4/12" },
...
const mainBlocks = ref([
    {
        points: [2424, 2214, 2718, 2476, 2723, 2479, 3403, 2203, 3407, 2156, 3488, 2123, 3594, 1336, 3583, 1332, 3602, 1167, 3140, 1077, 3115, 1081, 3009, 1056, 2943, 1064, 2936, 1100, 2926, 1096, 2832, 1108, 2832, 1108, 2832, 1111, 2728, 1124, 2730, 1130, 2672, 1134, 2670, 1134, 2669, 1144, 2461, 1169, 2458, 1318, 2458, 1320, 2430, 1324, 2422, 1297, 2408, 1298, 2403, 1321, 2405, 1330, 2388, 1331, 2363, 2112, 2364, 2115, 2426, 2167, 2425, 2179],
        block: {
            id: 1,
            name: "C",
        },
    ...
    {
        points: [1776, 1159, 1974, 1261, 2000, 1264, 2019, 1284, 2023, 1330, 2007, 1350, 2018, 1357, 2040, 1338, 2054, 1310, 2077, 1312, 2088, 1315, 2095, 1322, 2107, 1333, 2107, 1341, 2109, 1353, 2113, 1384, 2189, 1396, 2189, 1400, 2173, 1411, 2174, 1439, 2214, 1463, 2216, 1502, 2220, 1503, 2219, 1525, 2247, 1542, 2248, 1593, 2173, 1607, 2249, 1657, 2246, 1657, 2246, 1708, 2178, 1721, 2242, 1767, 2244, 1819, 2183, 1834, 2242, 1879, 2241, 1927, 2186, 1942, 2241, 1928, 2241, 1881, 2182, 1832, 2209, 1853, 2249, 1844, 2268, 1853, 2373, 1824, 2388, 1335, 2387, 1330, 2404, 1328, 2405, 1302, 2416, 1298, 2428, 1304, 2429, 1327, 2460, 1320, 2465, 1172, 2672, 1144, 2672, 1135, 2730, 1128, 2732, 1123, 2744, 1119, 2743, 1109, 2736, 1106, 2740, 987, 2500, 940, 1917, 985, 1916, 990, 1822, 998, 1821, 1113, 1778, 1118, 1779, 1155],
        block: {
            id: 2,
            name: "B",
        },
...
export function useProjectBanner() {
    ...
    floors.map(e => {
        ...
        if(planWidth && planHeight){
            e.tooltip_pos = {
                ...
                left: (e.pos === 'left') ? `${Math.round((1.05 * x_pos_max + 20) / (planWidth) * 100)}%` : `${Math.round((1-(x_pos_max * 0.78 / planWidth)) * 100)}%`,
            }
        }
    ...
    return {
        data: { blocks, floors, popupIsOpen, floorHovered, currentFloor, currentFloorNum },
